...
class Compute_Clusters():
	...
	def get_source_centers(self, G, C1, C2, train_data_batch_s, train_RRinterval_batch_s, train_prevRR_batch_s, train_prev_eightRR_batch_s, train_label_batch_s):

		#Calculate cluster-centroids for each class label using source domain data
		count_c_c=0
		cntr_gamma=0.5


		for i in range(int(train_data_batch_s.shape[0])):  
			...
			#For each class (0,1,2,3) label calculate cluster-centroids
			...
			idx_2_s=torch.where(label_batch_s==2)
			feat_2_s=feat_s[idx_2_s] 
			if feat_2_s.shape[0]==0:
				center_2_curr_s=torch.zeros(feat_2_s.shape[1])
			else:           
				center_2_curr_s=torch.mean(feat_2_s, 0) 
			center_2_curr_s=center_2_curr_s.to('cuda')

			idx_3_s=torch.where(label_batch_s==3)
			feat_3_s=feat_s[idx_3_s] 
			if feat_3_s.shape[0]==0:
				center_3_curr_s=torch.zeros(feat_3_s.shape[1])
			else:                         
				center_3_curr_s=torch.mean(feat_3_s, 0)                     
			center_3_curr_s=center_3_curr_s.to('cuda')
			...
			if count_c_c==0:
				...
				center_2_s=center_2_curr_s
				center_2_s=center_2_s.to('cuda')
				center_3_s=center_3_curr_s
				center_3_s=center_3_s.to('cuda')
			else:
				if feat_0_s.shape[0]==0:         
				...
				if feat_2_s.shape[0]==0:
					center_2_s=center_2_s
				else:
					center_2_s=(1-cntr_gamma) * center_2_s + cntr_gamma * center_2_curr_s  
				if feat_3_s.shape[0]==0:
					center_3_s=center_3_s
				else:             
					center_3_s=(1-cntr_gamma) * center_3_s + cntr_gamma * center_3_curr_s  
			count_c_c=count_c_c+1

		center_0_s=center_0_s.detach().cpu().data.numpy()
		center_1_s=center_1_s.detach().cpu().data.numpy()
		center_2_s=center_2_s.detach().cpu().data.numpy()
		center_3_s=center_3_s.detach().cpu().data.numpy()

		all_centers_s=np.empty((4,center_0_s.shape[0]))  
		all_centers_s[0]=center_0_s
		all_centers_s[1]=center_1_s
		all_centers_s[2]=center_2_s
		all_centers_s[3]=center_3_s

		return all_centers_s



	def get_mean_dist(self, G, C1, C2, train_data_batch_s, train_RRinterval_batch_s, train_prevRR_batch_s, train_prev_eightRR_batch_s, train_label_batch_s, all_centers_s):
		
		#Calculate mean intra-cluster distance and mean classifier discrepancy
		count_c_c=0
		cntr_gamma=0.5
		...
		for i in range(int(train_data_batch_s.shape[0])):  
			...
			count_correct_feat_2=0
			count_correct_feat_3=0
			for ii in range(correct_output_C1_prev_s.shape[0]):
     
				  sum_dist_classifiers=sum_dist_classifiers+ L2Distance(correct_output_C1_prev_s[ii], correct_output_C2_prev_s[ii])
      
				  count_correct_classifier=count_correct_classifier+1
				  
				  if correct_pred_batch_s[ii]==0:
					  sum_dist_feat_0=sum_dist_feat_0 + L2Distance(all_centers_s[0], correct_feat_s[ii])
					  count_correct_feat_0=count_correct_feat_0+1
				  if correct_pred_batch_s[ii]==1:
					  sum_dist_feat_1=sum_dist_feat_1 + L2Distance(all_centers_s[1], correct_feat_s[ii])
					  count_correct_feat_1=count_correct_feat_1+1
				  if correct_pred_batch_s[ii]==2:
					  sum_dist_feat_2=sum_dist_feat_2 + L2Distance(all_centers_s[2], correct_feat_s[ii])
					  count_correct_feat_2=count_correct_feat_2+1
				  if correct_pred_batch_s[ii]==3:
					  sum_dist_feat_3=sum_dist_feat_3 + L2Distance(all_centers_s[3], correct_feat_s[ii])
					  count_correct_feat_3=count_correct_feat_3+1
			...
			mean_dist_feat_2_curr_s= sum_dist_feat_2 / count_correct_feat_2
			mean_dist_feat_3_curr_s= sum_dist_feat_3 / count_correct_feat_3
			...
			if count_c_c==0:
				...
				mean_dist_feat_2_s=mean_dist_feat_2_curr_s
				mean_dist_feat_2_s=mean_dist_feat_2_s.to('cuda')
				mean_dist_feat_3_s=mean_dist_feat_3_curr_s
				mean_dist_feat_3_s=mean_dist_feat_3_s.to('cuda')
					...
					mean_dist_feat_2_s=(1-cntr_gamma) * mean_dist_feat_2_s + cntr_gamma * mean_dist_feat_2_curr_s 
					mean_dist_feat_3_s=(1-cntr_gamma) * mean_dist_feat_3_s + cntr_gamma * mean_dist_feat_3_curr_s 

			count_c_c=count_c_c+1
		mean_dist_feat_s = (mean_dist_feat_0_s + mean_dist_feat_1_s + mean_dist_feat_2_s + mean_dist_feat_3_s)/4   #Mean intra-cluster distance of all class labels
		mean_dist_classifier_s=mean_dist_classifier_s.detach().cpu().data.numpy()                                  #Mean classifier discrepancy
		...
		mean_dist_feat_2_s=mean_dist_feat_2_s.detach().cpu().data.numpy()
		mean_dist_feat_3_s=mean_dist_feat_3_s.detach().cpu().data.numpy()
		
		all_mean_dist_feat_s=np.empty((4))   #Mean intra-cluster distance for each (individual) class label
		all_mean_dist_feat_s[0]=mean_dist_feat_0_s
		all_mean_dist_feat_s[1]=mean_dist_feat_1_s
		all_mean_dist_feat_s[2]=mean_dist_feat_2_s
		all_mean_dist_feat_s[3]=mean_dist_feat_3_s
		

		return mean_dist_classifier_s, mean_dist_feat_s, all_mean_dist_feat_s


	def get_target_centers(self, G, C1, C2, train_data_batch_t, train_RRinterval_batch_t, train_prevRR_batch_t, train_prev_eightRR_batch_t,  all_centers_s, mean_dist_classifier_s, mean_dist_feat_s, all_mean_dist_feat_s):

		#Callculate confident predictions for target domain
		#Calculate cluster-centroids for each class label using confident predictions
		cntr_gamma=0.5
		count_c_c=0
		...
		for i in range(int(train_data_batch_t.shape[0])):  
			...
			confident_pred_idx_t=torch.where((max_pred_val_t>0.99))               #Pick initial confident predictions with softmax prediction>0.99
			...
			for ii in range(confident_feat_t.shape[0]):
				if ((L2Distance(confident_C1_t[ii], confident_C2_t[ii])<mean_dist_classifier_s)): #Check classifier discrepancy with previously calculated mean classifier discrepancy
			...
			for ii in range(feat_0_t.shape[0]):          
				if ((L2Distance(all_centers_s[0], feat_0_t[ii]) < all_mean_dist_feat_s[0])): #Check feature distance from previously calculated mean intra-cluster distance
			...
			for ii in range(feat_1_t.shape[0]):          
				if ((L2Distance(all_centers_s[1], feat_1_t[ii]) < all_mean_dist_feat_s[1])):  
			...
			idx_2_t=torch.where(confident_pred_class_t==2)  
			feat_2_t=confident_feat_t[idx_2_t] 
			conf_pred_2=confident_pred_class_t[idx_2_t]
			conf_pred_frac_2=confident_pred_output_t[idx_2_t]
			conf_C1_2=confident_C1_t[idx_2_t]
			conf_C2_2=confident_C2_t[idx_2_t]
			 
			jj=0
			for ii in range(feat_2_t.shape[0]):          
				if ((L2Distance(all_centers_s[2], feat_2_t[ii]) < all_mean_dist_feat_s[2])):  
					feat_2_t[jj]=feat_2_t[ii]
					conf_pred_2[jj]=conf_pred_2[ii]
					conf_pred_frac_2[jj]=conf_pred_frac_2[ii]
					conf_C1_2[jj]=conf_C1_2[ii]
					conf_C2_2[jj]=conf_C2_2[ii]
					jj=jj+1

			feat_2_t=feat_2_t[0:jj]
			conf_pred_2=conf_pred_2[0:jj]
			conf_pred_frac_2=conf_pred_frac_2[0:jj]
			conf_C1_2=conf_C1_2[0:jj]
			conf_C2_2=conf_C2_2[0:jj]


			if feat_2_t.shape[0]==0:
				center_2_curr_t=torch.zeros(feat_2_t.shape[1])
			else:           
				center_2_curr_t=torch.mean(feat_2_t, 0) 
			center_2_curr_t=center_2_curr_t.to('cuda')

			idx_3_t=torch.where(confident_pred_class_t==3)  
			feat_3_t=confident_feat_t[idx_3_t] 
			conf_pred_3=confident_pred_class_t[idx_3_t]
			conf_pred_frac_3=confident_pred_output_t[idx_3_t]
			conf_C1_3=confident_C1_t[idx_3_t]
			conf_C2_3=confident_C2_t[idx_3_t]

			jj=0
			for ii in range(feat_3_t.shape[0]):          
				if ((L2Distance(all_centers_s[3], feat_3_t[ii]) < all_mean_dist_feat_s[3])):   
					feat_3_t[jj]=feat_3_t[ii]
					conf_pred_3[jj]=conf_pred_3[ii]
					conf_pred_frac_3[jj]=conf_pred_frac_3[ii]
					conf_C1_3[jj]=conf_C1_3[ii]
					conf_C2_3[jj]=conf_C2_3[ii]
					jj=jj+1

			feat_3_t=feat_3_t[0:jj]
			conf_pred_3=conf_pred_3[0:jj]
			conf_pred_frac_3=conf_pred_frac_3[0:jj]
			conf_C1_3=conf_C1_3[0:jj]
			conf_C2_3=conf_C2_3[0:jj]

	  
			if feat_3_t.shape[0]==0:
				center_3_curr_t=torch.zeros(feat_3_t.shape[1])
			else:                         
				center_3_curr_t=torch.mean(feat_3_t, 0)                     
			center_3_curr_t=center_3_curr_t.to('cuda')
			...
			if count_c_c==0:
				...
				center_2_t=center_2_curr_t
				center_2_t=center_2_t.to('cuda')
				center_3_t=center_3_curr_t
				center_3_t=center_3_t.to('cuda')
			else:
				if feat_0_t.shape[0]==0:         
				...
				if feat_2_t.shape[0]==0:
					center_2_t=center_2_t
				else:
					center_2_t=(1-cntr_gamma) * center_2_t + cntr_gamma * center_2_curr_t 

				if feat_2_t.shape[0]==0:
					center_3_t=center_3_t
				else:             
					center_3_t=(1-cntr_gamma) * center_3_t + cntr_gamma * center_3_curr_t 

			count_c_c=count_c_c+1

		center_0_t=center_0_t.detach().cpu().data.numpy()
		center_1_t=center_1_t.detach().cpu().data.numpy()
		center_2_t=center_2_t.detach().cpu().data.numpy()
		center_3_t=center_3_t.detach().cpu().data.numpy()

		all_centers_t=np.empty((4,center_0_t.shape[0]))  
		all_count_t=np.empty((4)) 
		all_centers_t[0]=center_0_t
		all_centers_t[1]=center_1_t
		all_centers_t[2]=center_2_t
		all_centers_t[3]=center_3_t
